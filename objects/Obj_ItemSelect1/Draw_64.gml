if(!obj_GameController.inPlay)
{
	...
	if((position_meeting(window_mouse_get_x(), window_mouse_get_y(), id) || obj_mouse.itemSelect == id) && description != "")
	{
		...
		stringBuilder = "||";
		if(item_limit < 0) stringBuilder += item_name + " || ";
		else stringBuilder += item_name + ": " + string(item_amount) + " || ";
		stringBuilder += description + " Cost $" + string(cost);
		
		draw_text(obj_textBox.x + 50, obj_textBox.y, stringBuilder);
		*/
		
		
		draw_set_valign(fa_top);
		draw_set_color(c_yellow);
		stringBuilder = item_name +" Amount: " + string(item_amount);
